  ...
  function windowLoadInit() {
    ...
    if (jQuery().tweet) {
      jQuery('.twitter').tweet({
        ...
        template: "<span class=\"darklinks\">{user}</span><span class=\"tweet_text\">{tweet_text}</span><span class=\"greylinks\">{time}</span>"
      });
    }
    ...
    jQuery('.team-owl-carousel').each(function () {
      ...
      // 	initProgressbar(jQuery($owl2).find('.progress .progress-bar'));
      // })
    });
